import React, { useEffect, useState } from "react";
import { Card, CardBody } from "reactstrap";
import { gql, useQuery } from "@apollo/client";
import Iconify from "src/components/iconify/Iconify";

interface OrderSummary {
  pending: number;
  approvedToday: number;
  rejectedToday: number;
}

const Transactions = ({ vendorId }: any) => {
  const [orderSummary, setOrderSummary] = useState<OrderSummary>({
    pending: 0,
    approvedToday: 0,
    rejectedToday: 0,
  });

  const GET_ORDERS_SUMMARY = gql`
    query GetVendorDashboardReturnedOrderSummary {
      getVendorDashboardReturnedOrderSummary {
        pending
        approvedToday
        rejectedToday
      }
    }
  `;

  const { data: orderSummaryData, refetch: orderSummaryRefetch } = useQuery(GET_ORDERS_SUMMARY);

  useEffect(() => {
    if (orderSummaryData && orderSummaryData?.getVendorDashboardReturnedOrderSummary) {
      setOrderSummary(orderSummaryData?.getVendorDashboardReturnedOrderSummary)
    }
  }, [orderSummaryData, orderSummaryRefetch]);

  return (
    <React.Fragment>
      <Card style={{ height: '100%' }}>
        <div className="card-header align-items-center d-flex">
          <h4 className="card-title mb-0 flex-grow-1">Return Today</h4>
          <div className="flex-shrink-0"></div>
        </div>

        <CardBody className="px-12">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <div className="font-size-22 text-warning d-flex align-items-center">
              <Iconify icon="carbon:time-filled" width={22} />
              <h5 className="font-size-14 mb-0 ms-2"> Pending</h5>
            </div>
            <div className="text-end">
              <h5 className="font-size-14 text-muted mb-0">{orderSummary.pending}</h5>
              <p className="text-muted mb-0 font-size-12">Orders</p>
            </div>
          </div>

          <div className="d-flex justify-content-between align-items-center mb-3">
            <div className="font-size-22 text-success d-flex align-items-center">
              <Iconify icon="mdi:tick-circle" width={22} />
              <h5 className="font-size-14 mb-0 ms-2"> Approved</h5>
            </div>
            <div className="text-end">
              <h5 className="font-size-14 text-muted mb-0">{orderSummary.approvedToday}</h5>
              <p className="text-muted mb-0 font-size-12">Orders</p>
            </div>
          </div>

          <div className="d-flex justify-content-between align-items-center">
            <div className="font-size-22 text-danger d-flex align-items-center">
              <Iconify icon="carbon:close-filled" width={22} />
              <h5 className="font-size-14 mb-0 ms-2"> Rejected</h5>
            </div>
            <div className="text-end">
              <h5 className="font-size-14 text-muted mb-0">{orderSummary.rejectedToday}</h5>
              <p className="text-muted mb-0 font-size-12">Orders</p>
            </div>
          </div>
        </CardBody>
      </Card>
    </React.Fragment>
  );
};

export default Transactions;
